  ...
  function showInputTranslateDialog() {
    ...
    const translateInputText = async (targetLangOverride = '') => {
      ...
      try {
        ...
        // The dropdown writes its selection to dataset.targetLang, so that is
        // the language the header is currently showing — translate into what
        // the user can see, even if a settings change has since moved the
        // default out from under this open dialog. getInputTargetLang() is only
        // reached when there is no dropdown to read.
        const targetLang = targetLangOverride || state.inputDialog.dataset.targetLang || getInputTargetLang();
    ...
    if (ctx.setupLanguageDropdown) {
      ctx.setupLanguageDropdown(state.inputDialog, getInputTargetLang(), (lang) => {
