...
7. **动作行换字不挪位**（验收时主控改）：主控亲走时发现，动作行会折行（§6.1），按钮宽度又跟着字变。英文下点换引擎，按钮字从 `cardUseBuiltin` 变成较短的 `cardUseAi`，整行从两行并成一行，「复制」挪到了刚点过的位置下面。`settleCardActions` 在 `await` 之前先藏「换引擎」、之后再露出，每次结算整行闪一下。「复制 → 已复制」同样改宽度，而且复制反馈有两份实现（`content-popup.js` 的 `showCopyFeedback` 由划词卡和 OCR 卡共用，输入框对话框另写一份），都在点击那一刻存下 innerHTML、1.5 秒后放回：1.5 秒内连点两下，存下的就是「已复制」，按钮从此停在「已复制」。改法：
   - `ctx.fitLabel(text, labels)`（`content/content-utils.js`）把按钮会显示的每一种字写进 `data-fit`；`popup.css` 的 `.ai-translator-btn-label::after` 把它们排成一列看不见的影子，按钮按最宽的那种字定宽，换字只改 span 的 `textContent`。影子是伪元素，不进 `textContent`，也不进无障碍树。
   - `settleCardActions` 只在 `switchOffer` 回来之后设一次 `hidden`，中途不藏。
   - 两份复制反馈合成 `ctx.copyButtonContent(label)` + `ctx.copyWithFeedback(button, text)`：平时的字从按钮自己的 `data-fit` 读，计时器按按钮记在 WeakMap 里，连点只顺延。
   - `ctx.escapeHtml` 同时转义引号：它的结果本来就进属性值（语言菜单的 `data-tag`、划词图标的 `aria-label` / `title`），`data-fit` 又多一处，值里一个 `"` 就把属性截断了。现有文案里没有双引号，这一条是防御。
   - 守护：J-D9 在十种界面语言下，点换引擎、点复制前后，按钮在行内的位置和尺寸不变（±0.5px），行内没有按钮被藏过（MutationObserver），原点击点下仍是换引擎按钮。J-D1 和输入框对话框的 e2e 连点两下，等 2 秒后读一次，字已回到平时。`test/unit/copy-feedback.test.mjs` 扫描 `content/content-utils.js` 以外任何自己查「已复制」或自己写剪贴板的地方。六个变异（去掉影子、结算前先藏、影子不含「已复制」、恢复点击时存下的 innerHTML、另一处自查「已复制」、另一处自写剪贴板）全部被杀。
