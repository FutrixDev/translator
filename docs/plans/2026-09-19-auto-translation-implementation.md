...
**14. 那个窗不只看引擎，还要看回退（评审第 6 轮 P1）。** 第 4 条说「内置引擎不花
钱，所以不设窗」，漏了一层：`isActive()` 答的是「内置是选中的那个引擎，而且这个环境
给得了」，它答不了「这门语言对此刻真能在本机跑」。语言包还没下到本地时
`handleWithBuiltin()` 抛 `EngineUnavailableError`，而 `engineFallback === 'allow-ai'`
的用户会把这一批原样转给他自己的接口 —— 一场两小时的讲座在他看到第二句之前整片发去
了云端，正是那个窗要拦的事。不设窗的条件因此多一条：回退关着。反过来那种（回退开着
而包其实就在本地）会白设一个窗，代价只是译文晚一点点到，比前一种便宜得多。

**15. 缓存键要整码，不能砍成基码（评审第 6 轮 P2）。** 第 5 条把目标语言写进了
`getCueKey()`，但用的是 `getTargetLangBase()`：`zh-CN` 和 `zh-TW` 的基码都是 `zh`，
而它们是两套字。观众从简体切到繁体，旧译文的键一个不变，于是整段视频继续放着简体，
而且因为键「对」上了永远不会被重译掉 —— 和第 5 条要修的是同一个毛病，只是漏了一格。
新加 `getTargetLangKey()`（trim + 小写的整码）专作键用；`getTargetLangBase()` 留着，
它另有用处（判「原文就是目标语言，不必译」时按基码比才对）。

**16. 自动开成了要当场落闩，不能等下一拍（评审第 6 轮 P2）。** 第 3 条那道闩防的是
「观众关掉、我们点回来」，可心跳自己开成的那一下把 `true` 丢掉了：`state.sawNativeOn`
要等 1.5 秒后的下一拍才记上。观众在这 1.5 秒里把刚亮起来的字幕关掉，下一拍看见的是
「关着，而且没落闩」—— 又替他开一次，正是那道闩要防的事，只不过发生在它合上之前。
这条路上只记成功那一种：`false`（没有可开的字幕）**不记**，和菜单那条路正相反 ——
...
**18. 那个窗要按句子量，不能按批次量（评审第 8 轮 P2）。** 第 4/14 条设了窗，量的却
是整批离播放头多远 —— 而批次只按条数和字数切（`buildBatches`），时间上想多长有多长。
一段前面一句、一小时后一句的稀疏轨道，两句落在同一批里，这一批「最近的那一头」是 0，
于是一小时之外的那句跟着发了出去；密的轨道上一批 16 句也能让 30 秒的窗译出好几分钟。
改成按句子量（`segmentDistance`），一批里只把窗内那几句挑出来发，剩下的等窗滑过去。
`batchDistance` 一并删掉：同一个「距离」留两种量法，正是这条的来路。

**19. 「原字幕开着没有」拿不准的时候，不许去合那道闩（评审第 8 轮 P2）。** 那道闩
（第 3 条）一合就是一整个会话，而它的输入是一个会猜的探针：YouTube 的
`.ytp-caption-window-container` 是播放器外壳的一部分，可以先于控制条挂上来、而且是空
的。读成「开着」，等按钮带着 `aria-pressed="false"` 出现，下一拍就成了「观众刚把字幕
关掉」—— 自动开启从此停摆，而他什么都没做过。探针因此也改成三种答案（和第 13 条同一
套说法，provider 契约上只留 `nativeCaptionsState()` 这一个方法）：按钮是唯一的权威；
没有按钮时，容器里**真画着**字幕才算 true，空容器答 `null` 而**永远不答 false** ——
一来两句之间容器本来就是空的，二来这么弱的答案不该是合上那道闩的那一个。

**20. 那个窗每一轮现算，不能开场取一次（评审第 9 轮 P1）。** 第 14 条让窗看引擎也
看回退，可 `ensureTrackTranslated()` 是在循环外面取的一次 —— 一轮可以跑很久，而这中
间观众完全可以把引擎从内置换成 AI。开场那一下算出来的 `Infinity`（内置、不花钱、不
设窗）会一直用到这一轮跑完：换引擎那一下确实让在飞的那批变成 `STALE`（第 17 条），
可循环 `continue` 回去，拿的还是上一个引擎的那个结论，于是剩下整条轨道按「不设窗」
发给了云端。取窗的地方因此挪进循环：`pickNextBatch(translationWindowMs())`。一个会变
的答案留在手里，等于拿它上一次的值给下一次放行 —— 和第 5 条（key 在 await 之后算）
是同一个毛病的镜像：那边是算晚了，这边是算早了。

**21. 播放器的状态一概不记，每一拍现问（评审第 9 轮 P2）。** 第 7 条把「按了个空」
记进 `state.nativeUnavailable`，第 13 条给 `enableNativeCaptions()` 加了第三种答案
`null` 来护住这个标记 —— 两条都是在给一个不该存在的东西打补丁。YouTube 的 CC 按钮在
播放器加载中会**挂着但是 disabled**，此刻问它，诚实的答案就是 `false`（现在点不
开），而把这个答案记下来就是错的：它下一秒会变。第 19 条已经为探针得出过同一个结论
（拿不准不许合闩），这里是同一类 bug 的第三次，所以这次不是再打一个补丁，而是把记
的那张表删掉。

provider 契约因此变成三问一命令：`nativeCaptionsState()` 答此刻开着没有（三值，第 19
条）、`canEnableNativeCaptions()` 答此刻能不能点开（三值，`null` = 没有可依据的东
西）、`enableNativeCaptions()` 只管点，答真假。`state.nativeUnavailable` 全删，
`captionStatus()` 改成每一拍现问 `canEnableNativeCaptions()`，`!== false` 才露出
「开启原字幕」那一行 —— 按钮从 disabled 活过来，那一行自己就回来了，不必等字幕开起来
或者换视频。菜单那条路仍然不用自己造状态行（第 7 条的那一半还成立）：点空之后
`syncControls()` 照跑，只是这一拍的状态行是现问出来的，不是记账记出来的。

代价是 e2e 那条「没有字幕的视频不该给一个死按钮」变了行为：原先按一下、那一行消失，
现在是按钮 disabled 期间那一行**从不露出**，解除 disabled 之后自己回来。测试照新行为
改写，这不是放宽断言 —— 它现在多钉住了「回来」这一半。

**22. 「本来就是目标语言，不必译」也不能记（评审第 10 轮 P2）。** 和第 21 条同一
天、同一个毛病的第四次：`ingestTrack()` 按轨道语言和目标语言算一次
`state.skipTranslation`，而它一个视频只跑一次。观众看着一条英文轨道，把目标语言从英
文换成中文 —— 那个「不必译」冻在那里，之后每一次 `handleTimeUpdate()` 都在这道早退
上返回，整段视频再不会开译，除非播放器恰好重新交一次轨道进来。改成现算的
`sameLanguage()`（读 `state.trackLang` 和此刻的目标语言），字段整个删掉。

现算还差一步：得有人来推那一下。`targetLang` 从来不在 `CAPTION_SETTING_KEYS` 里 ——
它不是字幕自己的设置，可它一变，译文表的键（整码，第 15 条）和这道闸门一起翻篇。加
进那份名单，`applyCaptionSettings()` 顺手重渲染、重调度。而它调度那一下必须**越过 2
秒节流**：视频这时候多半是停着的，再没有一个 `timeupdate` 来推第二次（第 17 条踩过
同一个坑）。`handleTimeUpdate()` 因此收一个 `force`，`applyCaptionSettings()` 传
true。监听器不能再直接挂它 —— `addEventListener` 递进来的 Event 一概是真的，那样每
一次 `timeupdate` 都成了 force，节流等于没有；中间加一层 `onVideoTimeUpdate()`。

**23. 「本来就是目标语言」要排在「原字幕开着没有」后面（评审第 10 轮 P2）。** 第 9
条把轨道名排到了原字幕那一问后面，理由是关掉字幕不会抹掉上一轮留下的东西；同语言那
一句排得比轨道名还靠前，于是同一个理由在它身上又发作一次，而且更糟：一条本来就是目
标语言的轨道被观众关掉之后，菜单继续报「已经是你要的语言」—— 描述的是一条屏幕上已
经不存在的轨道，还正好把唯一那条回头路挡住了（自动开启那一面记着「是他自己关的」，
不会再替他点）。挪到原字幕那一问之后、轨道名之前。

**24. 「过期」这一问要排在看回包之前（评审第 11 轮 P2）。** 第 17 条把过期和失败分
开报了，可那一问排在三道失败判断**后面**，于是最先撞上的那条路漏掉：请求本身报错时
直接 `markBatchFailed` + `return false`，从不问「脚下的世界还是不是原来那个」。而换目
标语言那一刻在飞的请求多半两样都占（上游会把在飞的那一批连同它的通道一起放掉）——
按失败处理，就是记一笔谁也用不上的冷却，然后把整轮停在那里，而想接着译新那一套句子
的那次调用早被 `state.translating` 挡回去了。视频这时候停着，就再没有第二次。

改法不是在 catch 里补一份判断，而是把那一问整个提到 await 之后、看 response 之前：
成败先不管，世界变了就 STALE。三处失败判断因此都在它后面，`markBatchFailed()` 里那
道 `trackId` 复核也跟着删掉 —— 能走到它的批次现在必然不过期，同一个问题留两个答案迟
早会说成两回事。

**25. 那个窗只往前看（评审第 11 轮 P2）。** 第 18 条改成按句子量，用的
`segmentDistance()` 却是**对称**的：播放头后面五分钟的句子和前面五分钟的句子拿到同
一个距离，于是一起挤进同一份额度。观众往前跳进一段没译过的内容时，这个窗的实际宽度
是文档里写的两倍，而多出来的那一半全花在他刚跳过去的内容上。新增 `withinWindow()`：
`endMs < playheadMs` 的句子不占额度。

但不设窗那一路不受这条约束 —— 窗是**花钱的闸**，不是「译哪一段」的规矩。内置引擎
...
**27. 同语言那道闸门要比整码（评审第 13 轮 P2）。** 缓存键早就从基码改成了整码
（简繁是两套字），同一个道理漏在了 `sameLanguage()` 上：`zh-CN` 和 `zh-TW` 的基码
都是 `zh`，于是一条繁体轨道配简体目标被判成「本来就是目标语言」，`handleTimeUpdate()`
在那道闸门上返回，一个字也不译 —— 而「繁转简」正是观众要的那一件事。

判定挪进 `shared/caption-core.js` 的 `isSameLanguage()`：基码不同直接否；基码相同时
再看字（`zh-Hans`/`zh-CN`/`zh-SG` 对 `zh-Hant`/`zh-TW`/`zh-HK`/`zh-MO`）。**只有中文
有这一层**，`en-GB` 配 `en` 仍是同一门英语，去译一遍才是 bug。有一边说不准（光一个
`zh`）算同语言：这是一道花钱的闸，猜「不同」是替观众买一次多半什么也没变的翻译。引擎
这边顺手把 `getTargetLangKey()`/`getTargetLangBase()` 两个读数并成一个
`getTargetLang()` —— 键和闸门问的本来就是同一句话。

**留白（同一个类，本轮不动）**：整页那一面的 `isTargetLanguageText()` 也在拿基码比，
`zh-Hant` 的页面配 `zh-CN` 目标同样会被跳过。那一路的语言来自
`detectReliableLanguage()`，而它的返回值同时是自动翻译调度层用来对
`autoTranslateLangs`（基码表）的那一个 —— 改返回形状会连着动 PR-4/5 的判定，不属于本
PR。记在这里，别让它以「已经修过了」的样子留在那儿。

**28. 一轮译文得有主（评审第 13 轮 P2）。** `state.translating` 只是一面「有人在跑」
的旗子，而所有权是**会被收走的**：`resetForVideo()`（换视频、SPA 跳转）把它清掉，紧
接着新轨道进来 `ingestTrack()` 又起一轮新的，而旧那一轮正停在 await 上。它回来照第
17 条的 STALE 接着跑，两轮就并排跑起来了 —— 而且各自的 `finally` 都会把旗子清掉，于
是第三轮第四轮也能进来：付费的批次同时在飞，串行那条规矩等于没有。

改成拿号：`state.translating` 存的是**此刻这一轮是谁**。await 回来先对号，对不上就走
人、什么都不碰；`finally` 也只清自己那一号。换目标语言那一路不受影响 —— 没人动过所有
权，STALE 之后接着跑的还是这一轮，第 17 条的意思原样保留。

**29. 字幕的「过期」不挂在整页那一面的代次上（评审第 14 轮 P2）。** 第 17 条把在飞
的批次和「脚下的世界」对齐，用的却是调度层的 `sessionVersion` —— 那是**另一个部件的
时钟**。用户在 popup 上暂停这一页、把译文藏起来、关掉全局自动翻译，它都会翻篇，而字
幕的轨道、目标语言、引擎一样没变。在飞的那一批因此被判过期丢掉，下一轮
`pickNextBatch` 又把同一批没缓存的句子挑出来重发：钱付两回，而第一回的结果就在手上。

字幕这一面「同一回事」只有两样：哪条轨道，往哪门语言译。所以发请求那一刻连
`getTargetLang()` 一起取下，回来时比这两样，`sessionVersion()` 整个删掉。复用一个现
成的号看着省事，可它答的是别人的问题。

**30. 手里没轨道，不等于页面没开字幕（评审第 15 轮 P2）。** `TextTrackProvider` 的
`nativeCaptionsState()` 答的一直是「我们还攥着那条轨道没有」—— 我们把它按在 `hidden`
上自己画，播放器关字幕就是把它推回 `disabled`，这么读没错。错在字幕翻译**默认是关着
的**：那时这个 provider 只是个候选，`attach` 从来没跑过，`tt.track` 一直是 null。而
第 6 条早就把状态行接到了候选 provider 上 —— 于是一个 `<track>` 字幕压在画面上的页
面，菜单答的是「未检测到字幕轨」，因为功能关着时「开启原字幕」那一行本身是藏起来的
（`refreshMenu`），状态行只剩下那句最没用的话。

手里没有自己的轨道可读，就去读页面自己的：任何一条字幕轨不在 `disabled` 上，就是有人
在放字幕 —— 浏览器在 `showing` 上画，或者播放器自己在 `hidden` 上画。这对模式
`pickSubtitleTrack()` 早就在用了，所以判定收进 `CaptionCore.hasActiveSubtitleTrack()`
一处：挑选器和「原字幕开着没有」必须答得一样，不然 provider 会去问「要不要替你开字
幕」—— 而它正开着。

**31. 「开启原字幕」要开回他自己选的那条（评审第 15 轮 P2）。** 第 9 条让菜单那一行能
把观众自己关掉的字幕开回来，走的是 `syncSelection(true)`。可 `allowDisabled` 那几级台
阶是**挑**给「替我开字幕」用的：全部 `disabled` 时按「声道语言 → `default` → 第一条」
往下走。观众本来在看第四门语言、自己把字幕关了、再按这一行 —— 开回来的是页面列在最前
面的那门。一次改语言，穿着一次开字幕的衣服，而按下去的那一行正是要把他的选择还给他。

所以重开这条路先认手里还攥着的那条轨道（`heldEntry`），挑选器只在它不在了的时候接
手。「只在别的轨道都没开的时候」是那道让位：还有轨道停在 `showing`/`hidden` 上，说明
页面或观众已经换过一门了，那门才是当下的答案 —— 和 `pickSubtitleTrack()` 自己的次序
是同一句话。顺手把下面那道「我们还在这条轨道上」也改成用同一个 `heldEntry`：同一件事
算两遍，迟早会算出两个答案来。

**32. 页面换掉的那个 `<video>`，手里攥着的东西全得放开（评审第 16 轮 P2）。** 通用
SPA 在路由切换时把 `<video>` 整个换掉，而被摘走的那个元素**原样带着它的轨道**——脱离
文档、没人看得见，却照样有问必答。第 30 条让「手里有轨道就答那条轨道」，于是我们留在
旧元素上那条还停在 `hidden` 的轨道一口咬定「原字幕开着」：`autoEnableCaptions` 再也
不会替新视频开一条，`getVideo()` 把浮层框在一个谁也看不见的元素上，引擎手里那批 cue
还钉在一部放完了的片子上。新视频的轨道要是全在 `disabled`（`pickSubtitleTrack` 照第
1 条答 null），这一整套就永远卡在那里。

「手里的视频不是页面的了」有两种：元素被摘走（`isConnected === false`），或者元素还
在、而我们认得的那条轨道已经不在它的清单里。两道都排在 `syncSelection` 读任何东西之
前。放开的时候连引擎一起告诉：新视频上要是压根没发生过一次 adopt，没有别人会替它清掉
旧的 cue——`ingestTrack` 靠 trackId 翻篇，而那一路这时根本走不到。
...
**2. 「我能读的语言」是九个基码的勾选，没有自由输入。** 本文没说这份名单长什么
...
下拉里那几门走，`zh-CN`/`zh-TW` 在这里合成一个「中文」—— 这句话是「我读得懂
中文」，不是「翻成中文」，后者是上面那个下拉。为此新加了一个 `langZh` 文案键，
