...
本文是落地前写的。真写出来时有二十七处和上文不同 —— 这里逐条记下**为什么**，
...
同问同答；这一条的钉子除了源码守卫，还加了三个真跑 DOM 的行为测试
（`test/e2e/page-translation-restamp.spec.js`：语言不一样要换掉、同内容同语言仍然去重、
没人说语言也去重），两处变异（门改回只看 class、摘旧那步删掉）各自都能让第一条变红。

相应的钉子加在 `auto-translate-wiring.test.mjs`（现 30 条）和 `block-identity.test.mjs`
（第 17 条的三条行为测试：换语言判陈旧、两头都能退回从前、文字变了一票否决；第 27 条又添一条
落笔端守卫）。第 12 条还加了 e2e 的
