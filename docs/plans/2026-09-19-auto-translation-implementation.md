...
本文是落地前写的。真写出来时有二十三处和上文不同 —— 这里逐条记下**为什么**，
...
评审（Codex）在 PR #89 上跑了七轮，共 20 条，全部属实、全部已修。
...
相应的钉子加在 `auto-translate-wiring.test.mjs`（现 28 条）和 `block-identity.test.mjs`
（第 17 条的三条行为测试：换语言判陈旧、两头都能退回从前、文字变了一票否决）。第 12 条还加了 e2e 的
