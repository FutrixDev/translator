...
本文是落地前写的。真写出来时有十五处和上文不同 —— 这里逐条记下**为什么**，
...
评审（Codex）在 PR #89 上跑了四轮，共 12 条，全部属实、全部已修。
...
相应的钉子加在 `auto-translate-wiring.test.mjs`（现 20 条）。第 12 条还加了 e2e 的
...
那次比对换回 `fingerprint(block.text)`，`basic` 第一条挂在新补的那段带标记的正文上。
断言咬的是它们该咬的东西，不是碰巧变绿。
