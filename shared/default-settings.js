...
(function (root) {
  ...
  const CONTENT_DEFAULTS = Object.freeze({
    ...
    // 没人点也会发生的翻译（自动页面、视频字幕）一天最多发给模型多少字符。
    // 0 = 不限。
    //
    // 只有它们真能走到 AI 时才有意义：自动引擎选了 AI、字幕沿用的手动引擎选了
    // AI、或者开了回退（清单见 options/options-auto.js 的 unattendedAiReachable）。
    // 内置引擎不计费，也不记账。20 万字符大约是二三十篇长文，按主流小模型的价钱
